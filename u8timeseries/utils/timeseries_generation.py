...
def constant_timeseries(value: float = 1, length: int = 10, freq: str = 'D',
    ...
    :param freq: The time difference between two adjacent entries in the returned TimeSeries.
...
def sine_timeseries(value_frequency: float = 0.1, value_amplitude: float = 1, value_phase: float = 0,
    ...
    :param value_phase: The relative position within one period of the first value of the returned TimeSeries (radians).
...
def gaussian_timeseries(length: int = 10,
    ...
    if type(mean) == np.ndarray:
        raise_if_not(mean.shape == (length,),
                     'If a vector of means is provided, it requires the same length as the TimeSeries.', logger)
    if type(std) == np.ndarray:
        raise_if_not(std.shape == (length, length),
