...
void dtw_cost_matrix_no_window_1d(py::array_t<double> x, py::array_t<double> y, py::array_t<double> cost_matrix) {
    ...
    for (py::ssize_t i = 1; i <= x_ptr.shape(0); ++i) {
        for (py::ssize_t j = 1; j <= y_ptr.shape(0); ++j) {
            double cost = std::abs(x_ptr(i-1) - y_ptr(j-1));
            double min_prev_cost = std::min({cost_ptr(i-1, j), cost_ptr(i, j-1), cost_ptr(i-1, j-1)});
