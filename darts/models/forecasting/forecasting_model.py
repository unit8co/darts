                ...
                if (
                    ...
                    if start_time_ < historical_forecasts_time_index[0]:
                        logger.warning(
                            ...
                            f"forecasting point for series at index: {idx}. Ignoring `start` and beginning at time: "
                            f"{historical_forecasts_time_index[0]}."
                        )
                    else:
                        logger.warning(
                            f"`start` {start_value_msg} is after the last predictable/trainable historical "
                            f"forecasting point for series at index: {idx}. This results in empty historical "
                            f"forecasts. Non-empty forecasts can be generated by lowering `start` value to times "
                            f"before and including: {historical_forecasts_time_index[-1]}."
                        )
