...
import numpy as np
import pandas as pd
from statsmodels.regression.quantile_regression import QuantReg as staQuantReg
from statsmodels.tools.tools import add_constant

from darts.logging import get_logger, raise_if, raise_if_not
from darts.models.forecasting.forecasting_model import GlobalForecastingModel
from darts.models.forecasting.regression_model import _LikelihoodMixin
from darts.timeseries import TimeSeries

logger = get_logger(__name__)


class StaQuantileRegressionModel(GlobalForecastingModel, _LikelihoodMixin):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        """Regression Model
        Used to fit the statsmodels Quantile regressor class to predict the target time series from lagged values.
        ...
        model
            Statsmodels-like model with ``fit()`` and ``predict()`` methods. Also possible to use model that doesn't
        ...
        random_state
            ...
            `link <https://numpy.org/doc/stable/reference/random/generator.html#numpy.random.Generator>`_ .
            Default: ``None``.
        **kwargs
            Additional keyword arguments passed to `statsmodels` (by default), to
        ...
        raise_if(
            ...
            "Hey, At least one of `lags`, `lags_future_covariates` or `lags_past_covariates` must be not None.",
        )
    ...
    def __str__(self):
        return f"QuantileRegression(lags={self.lags})"
    ...
    def fit(
        ...
        **kwargs
        ...
        raise_if(
            "q" in kwargs,
            "quantiles are set when instantiating the model. Specify other parameters to QuantReg.fit() than 'q'",
        )
        ...
        self.model = staQuantReg(
            endog=training_labels,
            exog=add_constant(training_samples, has_constant="add"),
            **self.kwargs,
        )
        for quantile in self.quantiles:
            kwargs["q"] = quantile
            params = self.model.fit(**kwargs).params
            self._model_container[quantile] = self.model, params
        ...
        **kwargs : dict, optional
        ...
        for quantile, fitted in self._model_container.items():
            ...
            if isinstance(series, TimeSeries):
                ...
                series = [series]
                past_covariates = (
                    [past_covariates] if past_covariates is not None else None
                )
                future_covariates = (
            ...
            for t_pred in range(0, n, self.output_chunk_length):
                ...
                if "target" in self.lags:
                    target_matrix = (
                        np.concatenate([series_matrix, *predictions_q], axis=1)
                        if predictions_q
                        else series_matrix
                    )
                    np_X.append(
                ...
                prediction_q = self.model.predict(
                    params, exog=add_constant(X, has_constant="add")
                )
                # prediction_q = self.model.predict(params, exog=X)
                # reshape to (n_series, time (output_chunk_length), n_components)
                prediction_q = prediction_q.reshape(
                    len(series), self.output_chunk_length, -1
                )
                # append prediction to final predictions
                predictions_q.append(prediction_q)

            # concatenate and use first n points as prediction
            predictions_q = np.concatenate(predictions_q, axis=1)[:, :n]
            # build time series from the predicted values starting after end of series
            predictions_q = [
                self._build_forecast_series(row, input_tgt)
                for row, input_tgt in zip(predictions_q, series)
            ]

            prediction = (
                predictions_q[0] if called_with_single_series else predictions_q
            )
