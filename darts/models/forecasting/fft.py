...
class FFT(LocalForecastingModel):
    ...
    def _exp_trend(self, x) -> Callable:
        ...
        return np.exp(self.trend_coefficients[1]) * np.exp(self.trend_coefficients[0] * x)
    ...
    def fit(self, series: TimeSeries):
        ...
        # determine trend
        if self.trend == "poly":
            self.trend_coefficients = np.polyfit(range(len(series)), series.univariate_values(), self.trend_poly_degree)
            self.trend_function = self._poly_trend(self.trend_coefficients)
        elif self.trend == "exp":
            self.trend_coefficients = np.polyfit(range(len(series)), np.log(series.univariate_values()), 1)
