...
class NeuralProphet(DualCovariatesForecastingModel):
    def __init__(
        self,
        ...
        **prophet_kwargs,
        ...
        self._model_builder = neuralprophet.NeuralProphet
    ...
    def _predict(
        ...
        if num_samples == 1:
            forecast = self.model.predict(predict_df)["yhat1"].values
        else:
    ...
    def _generate_predict_df(
        ...
        predict_df = pd.DataFrame(
            data={"ds": self._generate_new_dates(n), "y": [None for _ in range(n)]}
        )
        if future_covariates is not None:
