...
class EnsembleModel(GlobalForecastingModel):
    ...
    def min_train_samples(self) -> int:
        ...
        # samples_in_output = extreme_lags[1] + 1 - self.output_chunk_shift
    ...
    def _target_window_lengths(self) -> tuple[int, int]:
        # the input length it is the maximum input length of the sub-models
        # the output length is what can be generated by the ensemble in one step + output_chunk_shift
        # note that for training, min_train_series_length requires some additional steps
        return abs(
            self.extreme_lags[0]
        ), self.output_chunk_shift + self.output_chunk_length
    ...
    def extreme_lags(
        ...
        if self.ensemble_model is not None:
            # either use the ensemble model's max target lag
