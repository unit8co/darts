...
class RegressionModel(GlobalForecastingModel):
    ...
    def _create_lagged_data(self, target_series, past_covariates, future_covariates, max_samples_per_ts):
        ...
        target_series = [target_series] if isinstance(target_series, TimeSeries) else target_series
        ...
        for idx, target_ts in enumerate(target_series):
            covariates = [
                (past_covariates[idx].pd_dataframe(), self.lags_past_covariates) if past_covariates else (None, None),
                ...
                if future_covariates else (None, None)
            ]
