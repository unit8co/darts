...
def _optimized_historical_forecasts_regression(
    ...
    for idx, series_ in enumerate(iterator):
        ...
        # TODO: might be able to optimize by removing X creation
        ...
        start = 0
            ...
            model.output_chunk_length - 1 if not model.multi_models else 0
        )
        new_series = []
        while end <= series_._time_index.shape[0] - forecast_horizon:
            new_series.append(series_[series_._time_index[start:end]])
            start += stride
            end += stride

        # TODO: need to handle past and future covariates here as well
