...
def _adjust_start(rel_start, stride):
    """If relative start position is negative, then adjust it to the first non-negative index that lies a
...
def _historical_forecasts_start_warnings(
    ...
    start_time: Union[int, pd.Timestamp],
    ...
    if not isinstance(start, pd.Timestamp):
        start_value_msg = f"value `{start}` corresponding to timestamp `{start_time}`"
    else:
        start_value_msg = f"time `{start_time}`"

    if start_time < historical_forecasts_time_index[0]:
        logger.warning(
