...
def _historical_forecasts_general_checks(model, series, kwargs):
    ...
    if n.start is not None:
        ...
        if not isinstance(n.start, (float, int, np.int64, pd.Timestamp)):
            raise_log(
                TypeError("`start` must be either `float`, `int`, `pd.Timestamp` or `None`."),
                logger,
            )

        if n.start_format not in ["position", "value"]:
            raise_log(ValueError(f"`start_format` must be on of ['position', 'value']. Received '{n.start_format}'."))
        ...
        for idx, series_ in enumerate(series):
            ...
            if isinstance(n.start, pd.Timestamp):
                ...
                if n.start_format == "position":
                    if (n.start > 0 and n.start >= len(series_)) or (n.start < 0 and np.abs(n.start) > len(series_)):
            ...
            if not overlap_end and start + (series_.freq * (n.forecast_horizon - 1)) not in series_:
                raise_log(
                    ValueError(
    ...
    if n.predict_likelihood_parameters:
        ...
        if model.output_chunk_length is not None and n.forecast_horizon > model.output_chunk_length:
            raise_log(
                ValueError(
...
def _historical_forecasts_sanitize_kwargs(
    ...
    if fit_kwargs is None:
        ...
        logger.warning("`fit_kwargs` was provided with `retrain=False`, the argument will be ignored.")
...
def _get_historical_forecastable_time_index(
    ...
    is_autoregression = max_target_lag >= 0 and forecast_horizon > max_target_lag - output_chunk_shift + 1
    ...
    if is_training:
        start = series.start_time() + (output_lag - output_chunk_shift - min_target_lag + 1) * series.freq
    else:
    ...
    if (min_past_cov_lag is not None) and (past_covariates is not None):
        if is_training:
            start_pc = (
                ...
                + (output_lag - output_chunk_shift - min_past_cov_lag + 1) * past_covariates.freq
            )
        else:
            start_pc = past_covariates.start_time() - min_past_cov_lag * past_covariates.freq
    ...
    if (min_future_cov_lag is not None) and (future_covariates is not None):
        if is_training:
            start_fc = (
                ...
                + (output_lag - output_chunk_shift - min_future_cov_lag + 1) * future_covariates.freq
            )
        else:
            start_fc = future_covariates.start_time() - min_future_cov_lag * future_covariates.freq
    ...
    if (
        not overlap_end
        and intersect_[1] + (forecast_horizon + output_chunk_shift - 1) * series.freq > series.end_time()
    ):
        intersect_ = (
            intersect_[0],
    ...
    if not reduce_to_bounds:
        intersect_ = generate_index(start=intersect_[0], end=intersect_[1], freq=series.freq)
...
def _adjust_historical_forecasts_time_index(
    ...
    if start is not None:
        ...
        if not historical_forecasts_time_index[0] <= start_time_ <= historical_forecasts_time_index[-1]:
            if show_warnings:
...
def _reconciliate_historical_time_indices(
    ...
    if isinstance(retrain, Callable):
        ...
        if (
            ...
            and historical_forecasts_time_index_train[0] < historical_forecasts_time_index_predict[0]
        ):
    ...
    if retrain or (not model._fit_called):
        ...
        # training samples, so we start after the first trainable point.
            ...
            if model.min_train_samples > 1:
                historical_forecasts_time_index = (
                    historical_forecasts_time_index[0] + (model.min_train_samples - 1) * series.freq,
                    historical_forecasts_time_index[1],
                )
...
def _check_optimizable_historical_forecasts_global_models(
    ...
    if retrain_off and (not is_autoregressive or (is_autoregressive and allow_autoregression)):
        return True
