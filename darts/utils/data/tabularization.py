...
def create_lagged_components_names(
    ...
    features_cols_name
        ...
        `features_cols_name` always returned as a `List[List[str]]` and if `concatenate=True`,
        each entry also contains the columns names for the `y` array (on the right).
    labels_cols_name
        The names of the lagged features in the `y` array generated by `create_lagged_data()`.
        `labels_cols_name` always returned as a `List[List[str]]`
    ...
    features_cols_name = []
    labels_cols_name = []
    for target_ts, past_cov_ts, future_cov_ts in zip_longest(
        target_series, past_covariates, future_covariates, fillvalue=None
    ):
        covariates_specs = []
        if target_ts and lags:
            covariates_specs.append((target_ts.components, lags))
        if past_cov_ts and lags_past_covariates:
            covariates_specs.append((past_cov_ts.components, lags_past_covariates))
        if future_cov_ts and lags_future_covariates:
            covariates_specs.append((future_cov_ts.components, lags_future_covariates))

        features_cols_name.append(
            [
        ...
        labels_cols_name.append(
            [
                f"lag_{lag_idx}_{comp_name}"
                for lag_idx in range(0, output_chunk_length)
                for comp_name in target_ts.components
            ]
        )

        if concatenate:
            features_cols_name[-1] += labels_cols_name[-1]
