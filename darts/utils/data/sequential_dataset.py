...
class SequentialDataset(TimeSeriesTrainingDataset):
    def __init__(self,
        ...
        series must have length at least `input_length + output_length`.
        ...
        if self.max_samples_per_ts is None:
            ...
            self.max_samples_per_ts = max(len(ts) for ts in self.target_series) - \
                                      self.output_length - self.input_length + 1
    ...
    def __getitem__(self, idx: int) -> Tuple[TimeSeries, TimeSeries, Optional[TimeSeries]]:
        ...
        n_samples_in_ts = len(ts_target) - self.input_length - self.output_length + 1
