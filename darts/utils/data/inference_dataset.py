...
class InferenceDataset(ABC, Dataset):
    ...
    def _covariate_indexer(
        ...
        if main_covariate_type not in [CovariateType.PAST, CovariateType.FUTURE]:
            raise_log(
                ValueError("`main_covariate_type` must be one of `(CovariateType.PAST, CovariateType.FUTURE)`"),
                logger=logger,
            )
        ...
        if main_covariate_type is CovariateType.PAST:
            future_end = past_end + max(0, n - output_chunk_length) * covariate_series.freq
        else:  # CovariateType.FUTURE
            # optionally, for future part of future covariates shift start and end by `output_chunk_shift`
            future_end = past_end + (max(n, output_chunk_length) + output_chunk_shift) * covariate_series.freq

        future_start = (
            past_end + covariate_series.freq * (1 + output_chunk_shift) if future_end != past_end else future_end
        )
...
class GenericInferenceDataset(InferenceDataset):
    def __init__(
        ...
        if (bounds is not None and stride == 0) or (bounds is None and stride > 0):
            raise_log(
                ValueError("Must supply either both `stride` and `bounds`, or none of them."),
                logger=logger,
            )
    ...
    def __getitem__(
        ...
        if not len(target_series) >= self.input_chunk_length:
            raise_log(
                ValueError(f"All input series must have length >= `input_chunk_length` ({self.input_chunk_length})."),
                logger=logger,
            )

        # extract past target values
        past_end = target_series.time_index[target_end_idx - 1 if target_end_idx is not None else -1]
        past_target = target_series.random_component_values(copy=False)[target_start_idx:target_end_idx]

        # optionally, extract covariates
        past_covariate, future_covariate = None, None
        covariate_series = None if self.covariates is None else self.covariates[series_idx]
