    ...
    def encode_train(
        ...
        **kwargs,
        ...
        merge_covariate
            Whether or not to merge the encoded TimeSeries with `covariate`.
        """
        # avoid pitfalls: `encode_train()` can be called multiple times. raise an error if the encoded component names
        # are already in `covariate` series
        if covariate is not None and self.fit_called:
            duplicate_components = self.components[
                self.components.isin(covariate.components)
            ]
            raise_if(
