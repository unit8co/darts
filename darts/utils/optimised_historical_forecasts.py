...
def _optimised_historical_forecasts_regression_last_points_only(
    ...
    """Optimized regression historical forecasts with last_points_only = True
...
def _optimised_historical_forecasts_regression_all_points(
    ...
    """Optimized regression historical forecasts with last_points_only = False"""
    forecasts_list = []
    for idx, series_ in enumerate(series):
        past_covariates_ = past_covariates[idx] if past_covariates is not None else None
        future_covariates_ = (
...
"""
def _optimised_historical_forecasts_regression(
    ...
    #Vectorise historical forecasts for regression based model
    ...
    for idx, series_ in enumerate(series):
        ...
        if (not last_points_only) and forecast_horizon > 1:
            if model.multi_models:
                shift_end = forecast_horizon - 1
                hist_fct_tgt_end += shift_end * freq
                hist_fct_pc_end += shift_end * freq
                hist_fct_fc_end += shift_end * freq
            else:
                shift_start = forecast_horizon - 1
                hist_fct_tgt_start -= shift_start * freq
                hist_fct_pc_start -= shift_start * freq
                hist_fct_fc_start -= shift_start * freq
        ...
        if last_points_only:
            ...
            if model.multi_models:
                if forecast_horizon > 1:
                    forecast = forecast[
                        :  -shift_end: stride, :forecast_horizon
                    ]
                else:
                    forecast = forecast[
