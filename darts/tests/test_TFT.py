...
if TORCH_AVAILABLE:
    class TFTModelTestCase(DartsBaseTestClass):
        ...
        def test_mixed_covariates_and_accuracy(self):
            ...
            season_length = 24
            n_repeat = 30
            ts, ts_train, ts_val, covariates = self.helper_generate_multivariate_case_data(season_length,
                                                                                           n_repeat)
        ...
        def helper_test_prediction_accuracy(self, predict_n, ts, ts_train, ts_val, past_covariates, future_covariates,
                                            kwargs_tft):
            ...
            y_hat = self.helper_fit_predict(predict_n,
                                            ts_train,
                                            ts_val,
                                            past_covariates,
                                            future_covariates,
            ...
            self.assertTrue(np.allclose(y_true[1:-1].all_values(),
                                        y_hat[1:-1].all_values(), atol=absolute_tolarance))
