...
class TimeSeriesTestCase(DartsBaseTestClass):
    ...
    def test_integer_indexing(self):
        ...
        self.assertTrue(
            np.all(
                series_int[pd.RangeIndex(start=10, stop=40, step=1)].univariate_values() == range_indexed_data[10:40]
            )
        )
        ...
        self.assertTrue(list(indexed_ts.time_index) == list(pd.RangeIndex(2, 7, step=1)))
    ...
    def helper_test_intersect(test_case, test_series: TimeSeries):
        ...
        seriesD = test_series.slice_intersect(
            TimeSeries.from_series(pd.Series(range(6, 13), index=pd.date_range("20130106", "20130112")))
        )
        ...
        seriesE = test_series.slice_intersect(
            TimeSeries.from_series(pd.Series(range(9, 13), index=pd.date_range("20130109", "20130112")))
        )
        ...
        with test_case.assertRaises(ValueError):
            test_series.slice_intersect(
                TimeSeries(pd.Series(range(6, 13), index=pd.date_range("20130116", "20130122")))
            )
    ...
    def test_rescale(self):
        ...
        seriesD = self.series3.rescale_with_value(1e20)  # TODO: test will fail if value > 1e24 due to num imprecision
    ...
    def helper_test_shift(test_case, test_series: TimeSeries):
        ...
        test_case.assertTrue(
            seriesB.time_index.equals(
                test_series.time_index[1:].append(pd.DatetimeIndex([test_series.time_index[-1] + test_series.freq]))
            )
        )
        ...
        test_case.assertTrue(
            seriesC.time_index.equals(
                pd.DatetimeIndex([test_series.time_index[0] - test_series.freq]).append(test_series.time_index[:-1])
            )
        )
    ...
    def test_getitem(self):
        ...
        self.assertTrue(
            ...
            == TimeSeries.from_dataframe(self.series1.pd_dataframe()[:1], freq=self.series1.freq)
        )
    ...
    def test_gaps(self):
        ...
        gaps1 = series1.gaps()
        self.assertTrue(
            (gaps1["gap_start"] == pd.DatetimeIndex([pd.Timestamp("20130103"), pd.Timestamp("20130109")])).all()
        )
        self.assertTrue(
            (gaps1["gap_end"] == pd.DatetimeIndex([pd.Timestamp("20130105"), pd.Timestamp("20130110")])).all()
        )
        ...
        self.assertTrue(
            (gaps5["gap_start"] == pd.DatetimeIndex([pd.Timestamp("20150101"), pd.Timestamp("20180101")])).all()
        )
        self.assertTrue(
            (gaps5["gap_end"] == pd.DatetimeIndex([pd.Timestamp("20160101"), pd.Timestamp("20190101")])).all()
        )
        gaps6 = series6.gaps()
...
class TimeSeriesConcatenateTestCase(DartsBaseTestClass):
    ...
    def test_concatenate_component_sunny_day(self):
        samples = [
            linear_timeseries(start_value=10, length=10, start=pd.Timestamp("2000-01-01"), freq="D"),
    ...
    def test_concatenate_time_different_time_axes_no_force_2_day_freq(self):
        samples = [
            linear_timeseries(start_value=10, length=10, start=pd.Timestamp("2000-01-01"), freq="2D"),
...
class TimeSeriesHeadTailTestCase(DartsBaseTestClass):
    ...
    def test_head_sunny_day_sample_axis(self):
        ...
        self.assertEqual(list(range(5, 10)), result._xa.coords["sample"].values.tolist())
...
class TimeSeriesFromDataFrameTestCase(DartsBaseTestClass):
    ...
    def test_time_col_convert_rangeindex(self):
        ...
        ar2 = data_dict["Values1"][list(expected_l.index(i) for i in range(len(expected)))]
...
class SimpleStatisticsTestCase(DartsBaseTestClass):
    ...
    def test_mean(self):
        for axis in range(3):
            ...
            # check values
            self.assertTrue(np.isclose(new_ts._xa.values, self.values.mean(axis=axis, keepdims=True)).all())

    def test_var(self):
        for ddof in range(5):
            new_ts = self.ts.var(ddof=ddof)
            # check values
            self.assertTrue(np.isclose(new_ts.values(), self.values.var(ddof=ddof, axis=2)).all())

    def test_std(self):
        for ddof in range(5):
            new_ts = self.ts.std(ddof=ddof)
            # check values
            self.assertTrue(np.isclose(new_ts.values(), self.values.std(ddof=ddof, axis=2)).all())
    ...
    def test_min(self):
        for axis in range(3):
            ...
            # check values
            self.assertTrue(np.isclose(new_ts._xa.values, self.values.min(axis=axis, keepdims=True)).all())

    def test_max(self):
        for axis in range(3):
            new_ts = self.ts.max(axis=axis)
            # check values
            self.assertTrue(np.isclose(new_ts._xa.values, self.values.max(axis=axis, keepdims=True)).all())

    def test_sum(self):
        for axis in range(3):
            new_ts = self.ts.sum(axis=axis)
            # check values
            self.assertTrue(np.isclose(new_ts._xa.values, self.values.sum(axis=axis, keepdims=True)).all())

    def test_median(self):
        for axis in range(3):
            new_ts = self.ts.median(axis=axis)
            # check values
            self.assertTrue(np.isclose(new_ts._xa.values, np.median(self.values, axis=axis, keepdims=True)).all())
