...
class TestTimeSeries:
    def test_creation(self):
        series_test = TimeSeries.from_series(pd_series1)
        assert series_test.pd_series().equals(pd_series1.astype(float))
        ...
        with pytest.raises(ValueError):
            ...
            ar3 = xr.DataArray(
                np.random.randn(10, 2, 1),
                dims=("time", "component", "sample"),
                coords={"time": times, "component": ["a", "a"]},
                name="time series",
            )
            _ = TimeSeries(ar3)
    ...
    def test_datetime_indexing(self):
        ...
        with pytest.raises(KeyError):
            series1[pd.Timestamp("20130111")]

        # slicing outside of the range should return an empty ts
        assert len(series1[pd.Timestamp("20130111") : pd.Timestamp("20130115")]) == 0
        assert series1[
            pd.Timestamp("20130111") : pd.Timestamp("20130115")
        ] == series1.slice(pd.Timestamp("20130111"), pd.Timestamp("20130115"))

        # slicing with an partial index overlap should return the ts subset (start and end included)
        assert len(series1[pd.Timestamp("20130105") : pd.Timestamp("20130112")]) == 6
        assert series1[
            pd.Timestamp("20130105") : pd.Timestamp("20130112")
        ] == series1.slice(pd.Timestamp("20130105"), pd.Timestamp("20130112"))
    ...
    def test_column_names(self):
        ...
        for cs_before, cs_after in zip(columns_before, columns_after):
            ar = xr.DataArray(
                ...
                dims=("time", "component", "sample"),
                coords={"time": times, "component": cs_before},
            )
    ...
    def test_alt_creation(self):
        with pytest.raises(ValueError):
            ...
            TimeSeries.from_times_and_values(
                index, pd_series1.values[:2], fill_missing_dates=True
            )
        with pytest.raises(ValueError):
            # all arrays must have same length
            TimeSeries.from_times_and_values(pd_series1.index, pd_series1.values[:-1])
        ...
        series_test = TimeSeries.from_times_and_values(
            index, pd_series1.values[rand_perm - 1]
        )

        assert series_test.start_time() == pd.to_datetime("20130101")
        assert series_test.end_time() == pd.to_datetime("20130110")
        assert np.all(series_test.pd_series().values == pd_series1.values)
        assert series_test.freq == series1.freq

    # TODO test over to_dataframe when multiple features choice is decided

    def test_eq(self):
        seriesA: TimeSeries = TimeSeries.from_series(pd_series1)
        assert series1 == seriesA
        assert not series1 != seriesA

        # with different dates
        seriesC = TimeSeries.from_series(
            pd.Series(range(10), index=pd.date_range("20130102", "20130111"))
        )
        assert not series1 == seriesC

    def test_dates(self):
        assert series1.start_time() == pd.Timestamp("20130101")
        assert series1.end_time() == pd.Timestamp("20130110")
        assert series1.duration == pd.Timedelta(days=9)

    def helper_test_slice(self, test_series: TimeSeries):
        # base case
        ...
        assert (sliced_idx == pd.RangeIndex(7, 9)).all()
    ...
    def helper_test_split(self, test_series: TimeSeries):
    ...
    def helper_test_drop(self, test_series: TimeSeries):
    ...
    def helper_test_intersect(self, test_series: TimeSeries):
    ...
    def test_rescale(self):
        with pytest.raises(ValueError):
            series1.rescale_with_value(1)

        seriesA = series3.rescale_with_value(0)
        assert np.all(seriesA.values() == 0)

        seriesB = series3.rescale_with_value(-5)
        assert series3 * -1.0 == seriesB

        seriesC = series3.rescale_with_value(1)
        assert series3 * 0.2 == seriesC

        seriesD = series3.rescale_with_value(
            1e20
        )  # TODO: test will fail if value > 1e24 due to num imprecision
        assert series3 * 0.2e20 == seriesD

    def helper_test_shift(self, test_series: TimeSeries):
        seriesA = series1.shift(0)
        assert seriesA == series1
    ...
    def helper_test_append(self, test_series: TimeSeries):
        # reconstruct series
    ...
    def helper_test_append_values(self, test_series: TimeSeries):
        # reconstruct series
        seriesA, seriesB = test_series.split_after(pd.Timestamp("20130106"))
    ...
    def helper_test_prepend(self, test_series: TimeSeries):
        # reconstruct series
        seriesA, seriesB = test_series.split_after(pd.Timestamp("20130106"))
    ...
    def helper_test_prepend_values(self, test_series: TimeSeries):
        # reconstruct series
        seriesA, seriesB = test_series.split_after(pd.Timestamp("20130106"))
    ...
    def test_slice(self):
        self.helper_test_slice(series1)

    def test_split(self):
        self.helper_test_split(series1)

    def test_drop(self):
        self.helper_test_drop(series1)

    def test_intersect(self):
        self.helper_test_intersect(series1)

    def test_shift(self):
        self.helper_test_shift(series1)

    def test_append(self):
        self.helper_test_append(series1)
    ...
    def test_append_values(self):
        self.helper_test_append_values(series1)
    ...
    def test_prepend(self):
        self.helper_test_prepend(series1)
    ...
    def test_prepend_values(self):
        self.helper_test_prepend_values(series1)
    ...
    def test_diff(self):
        diff1 = TimeSeries.from_dataframe(series1.pd_dataframe().diff())
        ...
        diff_shift2 = TimeSeries.from_dataframe(series1.pd_dataframe().diff(periods=2))
        diff_shift2_no_na = TimeSeries.from_dataframe(
            series1.pd_dataframe().diff(periods=2).dropna()
        )
        ...
        with pytest.raises(ValueError):
            series1.diff(n=0.2)
        with pytest.raises(ValueError):
            series1.diff(periods=0.2)

        assert series1.diff() == diff1_no_na
        assert series1.diff(n=2, dropna=True) == diff2_no_na
        assert series1.diff(dropna=False) == diff1
        assert series1.diff(n=2, dropna=0) == diff2
        assert series1.diff(periods=2, dropna=True) == diff_shift2_no_na
        assert series1.diff(n=2, periods=2, dropna=False) == diff2_shift2

    def test_ops(self):
        seriesA = TimeSeries.from_series(
            pd.Series([2 for _ in range(10)], index=pd_series1.index)
        )
        targetAdd = TimeSeries.from_series(
            pd.Series(range(2, 12), index=pd_series1.index)
        )
        targetSub = TimeSeries.from_series(
            pd.Series(range(-2, 8), index=pd_series1.index)
        )
        targetMul = TimeSeries.from_series(
            pd.Series(range(0, 20, 2), index=pd_series1.index)
        )
        targetDiv = TimeSeries.from_series(
            pd.Series([i / 2 for i in range(10)], index=pd_series1.index)
        )
        targetPow = TimeSeries.from_series(
            pd.Series([float(i**2) for i in range(10)], index=pd_series1.index)
        )

        assert series1 + seriesA == targetAdd
        assert series1 + 2 == targetAdd
        assert 2 + series1 == targetAdd
        assert series1 - seriesA == targetSub
        assert series1 - 2 == targetSub
        assert series1 * seriesA == targetMul
        assert series1 * 2 == targetMul
        assert 2 * series1 == targetMul
        assert series1 / seriesA == targetDiv
        assert series1 / 2 == targetDiv
        assert series1**2 == targetPow
    ...
    def test_getitem_datetime_index(self):
        seriesA: TimeSeries = series1.drop_after(pd.Timestamp("20130105"))
        assert series1[pd.date_range("20130101", " 20130104")] == seriesA
        assert series1[:4] == seriesA
        assert series1[pd.Timestamp("20130101")] == TimeSeries.from_dataframe(
            series1.pd_dataframe()[:1], freq=series1.freq
        )
        assert series1[pd.Timestamp("20130101") : pd.Timestamp("20130104")] == seriesA

        with pytest.raises(KeyError):
            series1[pd.date_range("19990101", "19990201")]
    ...
    def test_fill_missing_dates(self):
        ...
        assert math.isnan(series_test.pd_series().get("20130105")) is True
    ...
    def test_short_series_slice(self):
        seriesA, seriesB = series1.split_after(pd.Timestamp("20130108"))
        assert len(seriesA) == 8
        assert len(seriesB) == 2
        seriesA, seriesB = series1.split_after(pd.Timestamp("20130109"))
        assert len(seriesA) == 9
        assert len(seriesB) == 1
        assert seriesB.time_index[0] == series1.time_index[-1]
        seriesA, seriesB = series1.split_before(pd.Timestamp("20130103"))
        assert len(seriesA) == 2
        assert len(seriesB) == 8
        seriesA, seriesB = series1.split_before(pd.Timestamp("20130102"))
        assert len(seriesA) == 1
        assert len(seriesB) == 9
        assert seriesA.time_index[-1] == series1.time_index[0]
        seriesC = series1.slice(pd.Timestamp("20130105"), pd.Timestamp("20130105"))
    ...
    def test_map(self):
        ...
        assert not series_01 == series[["0", "1"]].map(fn)
...
# TimeSeriesConcatenateTestCase
#
# COMPONENT AXIS TESTS
#


def test_concatenate_component_sunny_day():
    samples = [
        linear_timeseries(
...
def test_concatenate_component_different_time_axes_no_force():
    samples = [
        linear_timeseries(
            start_value=10, length=10, start=pd.Timestamp("2000-01-01"), freq="D"
        ),
...
def test_concatenate_component_different_time_axes_with_force():
    samples = [
        linear_timeseries(
            start_value=10, length=10, start=pd.Timestamp("2000-01-01"), freq="D"
        ),
...
def test_concatenate_component_different_time_axes_with_force_uneven_series():
    samples = [
        linear_timeseries(
            start_value=10, length=10, start=pd.Timestamp("2000-01-01"), freq="D"
        ),
...
def test_concatenate_sample_sunny_day():
    samples = [
        linear_timeseries(
            start_value=10, length=10, start=pd.Timestamp("2000-01-01"), freq="D"
        ),
...
def test_concatenate_time_sunny_day():
    samples = [
        linear_timeseries(
            start_value=10, length=10, start=pd.Timestamp("2000-01-01"), freq="D"
        ),
...
def test_concatenate_time_same_time_no_force():
    samples = [
        linear_timeseries(
            start_value=10, length=10, start=pd.Timestamp("2000-01-01"), freq="D"
        ),
...
def test_concatenate_time_same_time_force():
    samples = [
        linear_timeseries(
            start_value=10, length=10, start=pd.Timestamp("2000-01-01"), freq="D"
        ),
...
def test_concatenate_time_different_time_axes_no_force():
    samples = [
        linear_timeseries(
            start_value=10, length=10, start=pd.Timestamp("2000-01-01"), freq="D"
        ),
...
def test_concatenate_time_different_time_axes_force():
    samples = [
        linear_timeseries(
            start_value=10, length=10, start=pd.Timestamp("2000-01-01"), freq="D"
        ),
...
def test_concatenate_time_different_time_axes_no_force_2_day_freq():
    samples = [
        linear_timeseries(
...
def test_concatenate_timeseries_method():
...
def test_creation_with_hierarchy_sunny_day():
    hierarchical_series = TimeSeries.from_values(
        values=np.random.rand(50, len(components), 5),
        columns=components,
        hierarchy=hierarchy,
    )
    assert hierarchical_series.hierarchy == hierarchy


def test_with_hierarchy_sunny_day():
    hierarchical_series = base_series.with_hierarchy(hierarchy)
    assert hierarchical_series.hierarchy == hierarchy


def test_with_hierarchy_rainy_day():
    # wrong type
    with pytest.raises(ValueError):
        base_series.with_hierarchy(set())

    # wrong keys
    with pytest.raises(ValueError):
        hierarchy = {"ax": ["a", "x"]}
        base_series.with_hierarchy(hierarchy)
...
def test_concat():
    series1 = base_series.with_hierarchy(hierarchy)
    series2 = base_series.with_hierarchy(hierarchy)

    # concat on time or samples should preserve hierarchy:
    concat_s = concatenate([series1, series2], axis=0, ignore_time_axis=True)
    assert concat_s.hierarchy == hierarchy
...
def test_ops_hierarchy():
    ...
    series1 = base_series.with_hierarchy(hierarchy)
    series2 = base_series.with_hierarchy(hierarchy2)

    assert series1[:10].hierarchy == hierarchy
    assert (series1 + 10).hierarchy == hierarchy

    # combining series should keep hierarchy of first series
    assert (series1 / series2).hierarchy == hierarchy
    assert (series1.slice_intersect(series2[10:20])).hierarchy == hierarchy


def test_with_string_items():
...
# TimeSeriesHeadTailTestCase
def test_head_sunny_day_time_axis():
    result = ts.head()
    assert 5 == result.n_timesteps
    assert pd.Timestamp("2000-01-05") == result.end_time()


def test_head_sunny_day_component_axis():
    result = ts.head(axis=1)
...
def test_tail_sunny_day_time_axis():
    result = ts.tail()
    assert 5 == result.n_timesteps
    assert pd.Timestamp("2000-01-06") == result.start_time()


def test_tail_sunny_day_component_axis():
    result = ts.tail(axis=1)
    assert 5 == result.n_components
...
def test_head_sunny_day_sample_axis():
    result = ts.tail(axis=2)
    assert 5 == result.n_samples
    assert list(range(5, 10)) == result._xa.coords["sample"].values.tolist()


def test_head_overshot_time_axis():
    result = ts.head(20)
    assert 10 == result.n_timesteps
    assert pd.Timestamp("2000-01-10") == result.end_time()


def test_head_overshot_component_axis():
    result = ts.head(20, axis="component")
    assert 10 == result.n_components


def test_head_overshot_sample_axis():
    result = ts.head(20, axis="sample")
    assert 10 == result.n_samples


def test_head_numeric_time_index():
    s = TimeSeries.from_values(ts.values())
    # taking the head should not crash
    s.head()


def test_tail_overshot_time_axis():
    result = ts.tail(20)
    assert 10 == result.n_timesteps
    assert pd.Timestamp("2000-01-01") == result.start_time()


def test_tail_overshot_component_axis():
    result = ts.tail(20, axis="component")
    assert 10 == result.n_components


def test_tail_overshot_sample_axis():
    result = ts.tail(20, axis="sample")
    assert 10 == result.n_samples


def test_tail_numeric_time_index():
    s = TimeSeries.from_values(ts.values())
    # taking the tail should not crash
    s.tail()


# TimeSeriesFromDataFrameTestCase
def test_from_dataframe_sunny_day():
    data_dict = {"Time": pd.date_range(start="20180501", end="20200301", freq="MS")}
    data_dict["Values1"] = np.random.uniform(
        low=-10, high=10, size=len(data_dict["Time"])
    )
...
def test_time_col_convert_string_integers():
...
def test_time_col_convert_integers():
...
def test_fail_with_bad_integer_time_col():
...
def test_time_col_convert_rangeindex():
...
def test_time_col_convert_datetime():
...
def test_time_col_convert_datetime_strings():
    expected = pd.date_range(start="20180501", end="20200301", freq="MS")
...
def test_time_col_convert_garbage():
    expected = [
        "2312312asdfdw",
        "asdfsdf432sdf",
        "sfsdfsvf3435",
...
def test_mean():
    for axis in range(3):
        new_ts = ts.mean(axis=axis)
        # check values
        assert np.isclose(
            new_ts._xa.values, values.mean(axis=axis, keepdims=True)
        ).all()


def test_var():
    for ddof in range(5):
        new_ts = ts.var(ddof=ddof)
        # check values
        assert np.isclose(new_ts.values(), values.var(ddof=ddof, axis=2)).all()


def test_std():
    for ddof in range(5):
        new_ts = ts.std(ddof=ddof)
        # check values
        assert np.isclose(new_ts.values(), values.std(ddof=ddof, axis=2)).all()


def test_skew():
    new_ts = ts.skew()
    # check values
    assert np.isclose(new_ts.values(), skew(values, axis=2)).all()
...
def test_kurtosis():
    ...
    assert np.isclose(
        new_ts.values(),
        kurtosis(values, axis=2),
    ).all()


def test_min():
    for axis in range(3):
        new_ts = ts.min(axis=axis)
        # check values
        assert np.isclose(new_ts._xa.values, values.min(axis=axis, keepdims=True)).all()


def test_max():
    for axis in range(3):
        new_ts = ts.max(axis=axis)
        # check values
        assert np.isclose(new_ts._xa.values, values.max(axis=axis, keepdims=True)).all()


def test_sum():
    for axis in range(3):
        new_ts = ts.sum(axis=axis)
        # check values
        assert np.isclose(new_ts._xa.values, values.sum(axis=axis, keepdims=True)).all()


def test_median():
    for axis in range(3):
        new_ts = ts.median(axis=axis)
        # check values
        assert np.isclose(
            new_ts._xa.values, np.median(values, axis=axis, keepdims=True)
        ).all()


def test_quantile():
    for q in [0.01, 0.1, 0.5, 0.95]:
        new_ts = ts.quantile(quantile=q)
        # check values
        assert np.isclose(
            new_ts.values(),
