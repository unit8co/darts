...
list_detector = list_Detector + list_FittableDetector
...
class TestAnomalyDetectionDetector:
    ...
    def test_QuantileDetector(self):
        ...
        assert np.abs(9.13658 - detector.low_threshold[0]) < delta

        # detector parameter 'abs_high_' must be equal to 10.74007 when trained on the series 'train'
        assert np.abs(10.74007 - detector.high_threshold[0]) < delta
        ...
        assert (
            np.abs(
                0.57
                - detector.eval_metric(self.anomalies, self.test, metric="accuracy")
            )
            < delta
        )
        ...
        assert (
            np.abs(
                0.4 - detector.eval_metric(self.anomalies, self.test, metric="recall")
            )
            < delta
        )
        ...
        assert (
            np.abs(
                0.08510 - detector.eval_metric(self.anomalies, self.test, metric="f1")
            )
            < delta
        )
        ...
        assert (
            np.abs(
                0.04761
                - detector.eval_metric(self.anomalies, self.test, metric="precision")
            )
            < delta
        )
        ...
        assert (
            len(
                detector.eval_metric(self.mts_anomalies, self.mts_test, metric="recall")
            )
            == 2
        )
        assert (
            len(detector.eval_metric(self.mts_anomalies, self.mts_test, metric="f1"))
            == 2
        )
        assert (
            len(
        ...
        acc = detector.eval_metric(self.mts_anomalies, self.mts_test, metric="accuracy")
        assert np.abs(0.69 - acc[0]) < delta
        assert np.abs(0.75 - acc[1]) < delta

        precision = detector.eval_metric(
            self.mts_anomalies, self.mts_test, metric="precision"
        )
        assert np.abs(0.0 - precision[0]) < delta
        assert np.abs(0.10526 - precision[1]) < delta

        recall = detector.eval_metric(
            self.mts_anomalies, self.mts_test, metric="recall"
        )
        assert np.abs(0.0 - recall[0]) < delta
        assert np.abs(0.2 - recall[1]) < delta

        f1 = detector.eval_metric(self.mts_anomalies, self.mts_test, metric="f1")
        assert np.abs(0.0 - f1[0]) < delta
        assert np.abs(0.13793 - f1[1]) < delta
    ...
    def test_ThresholdDetector(self):
        ...
        assert (
            np.abs(
                0.06349 - detector.eval_metric(self.anomalies, self.test, metric="f1"),
            )
            < delta
        )
