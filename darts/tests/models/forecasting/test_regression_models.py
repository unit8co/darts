...
class ProbabilisticRegressionModelsTestCase(DartsBaseTestClass):
    ...
    def test_static_cov_shap_explainer(self):
        """
        Tests that `RandomForest` regression model reproduces same behaviour as
        `examples/15-static-covariates.ipynb` notebook; see this notebook for
        futher details. Notebook is also hosted online at:
        ...
        sine_vals[61:80] = linear_vals
        irregular_series = TimeSeries.from_times_and_values(
            values=sine_vals, times=sine_series.time_index, columns=["value"]
        )
        ...
        shap_explain_no_stat_covs = ShapExplainer(model_no_static_cov, background_series=fitting_series)
        explanation_results_no_static_covs = shap_explain_no_stat_covs.explain()
        ...
        shap_explain = ShapExplainer(model_static_cov, background_series=fitting_series)
        explanation_results = shap_explain.explain()
        from matplotlib import pyplot as plt
        test = 1


    def test_static_cov_shap_explainer_fails(self):
        """
        Tests that `RandomForest` regression model reproduces same behaviour as
        `examples/15-static-covariates.ipynb` notebook; see this notebook for
        futher details. Notebook is also hosted online at:
