...
class RegressionModelsTestCase(DartsBaseTestClass):
        ...
        for model_cls in [RegressionModel, LinearRegressionModel, LightGBMModel]:
            ...
            for model, ex in zip(
                ...
                self.helper_compare_encoded_covs_with_ref(
                    model_copy, ts, covariates, n=1
                )

                _ = model_copy.predict(n=3, series=ts)
                self.helper_compare_encoded_covs_with_ref(
                    model_copy, ts, covariates, n=3
                )

                _ = model_copy.predict(n=8, series=ts)
                self.helper_compare_encoded_covs_with_ref(
                    model_copy, ts, covariates, n=8
                )
    ...
    @staticmethod
    def helper_compare_encoded_covs_with_ref(model, ts, covariates, n):
        ...
        def generate_expected_times(ts, n_predict=0) -> dict:
            ...
            if not n_predict:
                ...
                pc1_start, pc1_end = (
                    to_ts("2000-09-01"),
                    to_ts("2000-11-01") + freq * n_predict,
                )
                pc2_start, pc2_end = (
                    to_ts("2001-09-01"),
                    to_ts("2001-11-01") + freq * n_predict,
                )
                fc1_start, fc1_end = (
                    to_ts("2001-01-01"),
                    to_ts("2001-04-01") + freq * n_predict,
                )
                fc2_start, fc2_end = (
                    to_ts("2002-01-01"),
                    to_ts("2002-04-01") + freq * n_predict,
                )
