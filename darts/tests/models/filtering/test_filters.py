...
class TestMovingAverage(FilterBaseTestClass):
    ...
    def test_moving_average_multivariate(self):
        ...
        assert np.mean(np.abs(ts.values()[:, 0])) > np.mean(np.abs(ts_filtered.values()[:, 0]))
        assert np.mean(np.abs(ts.values()[:, 1])) > np.mean(np.abs(ts_filtered.values()[:, 1]))
...
class TestGaussianProcessFilter(FilterBaseTestClass):
    def test_gaussian_process(self):
        ...
        gpf = GaussianProcessFilter(kernel=kernel, alpha=0.2, n_restarts_optimizer=10, random_state=42)
