...
class GetSharedTimesBoundsTestCase(DartsBaseTestClass):
    ...
    def test_shared_times_bounds_time_idx_inputs(self):
        ...
        self.assertEqual(
            get_shared_times_bounds(series_1.time_index),
            (series_1.start_time(), series_1.end_time()),
        )
        # Pass only `series_2.time_index` - bounds are just start and
        # end times of this series:
        self.assertEqual(
            get_shared_times_bounds(series_2.time_index),
            (series_2.start_time(), series_2.end_time()),
        )
        self.assertEqual(
            get_shared_times_bounds(series_1.time_index, series_2),
            expected_bounds,
        )
        self.assertEqual(
            get_shared_times_bounds(series_1, series_2.time_index),
            expected_bounds,
        )
        self.assertEqual(
    ...
    def test_shared_times_bounds_subset_series_range_idx(self):
        ...
        self.assertEqual(
            get_shared_times_bounds(series, subseries, subsubseries),
            expected_bounds,
        )
