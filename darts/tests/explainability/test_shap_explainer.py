...
class TestShapExplainer:
    ...
    @pytest.mark.parametrize(
        "univariate,model_cls",
        itertools.product([True, False], [LinearRegressionModel, XGBModel]),
    )
    def test_compare_multi_models_shap_values(
        self, univariate: bool, model_cls: RegressionModel
    ):
        """
        ...
        """
        target_component = "price"
        if univariate:
            ts = self.target_ts[target_component]
        else:
            ts = self.target_ts

        models = {
            "multi_models": model_cls(
                lags=6, output_chunk_length=3, multi_models=True, random_state=1
            ),
            "single_model": model_cls(
                lags=6, output_chunk_length=3, multi_models=False, random_state=1
            ),
        }
        ...
        for model_name, model in models.items():
            ...
            # check force_plot can be called
            explainer.force_plot_from_ts(
                ts[:8], show=False, target_component=target_component
            )
            # check explain can be called
            explainer.explain(ts[:8], target_components=target_component)
            # compute and store shap values
            shap_values[model_name] = explainer.summary_plot(
                show=False, target_components=target_component
            )

        # shap values should be identical for the last horizon step
        np.testing.assert_almost_equal(
            shap_values["multi_models"][-1].values,
            shap_values["single_model"][-1].values,
        )
        # input features should be identical for the last horizon step
        np.testing.assert_almost_equal(
            shap_values["multi_models"][-1].data, shap_values["single_model"][-1].data
        )
        # base values should be identical for the last horizon step
        np.testing.assert_almost_equal(
            shap_values["multi_models"][-1].base_values,
            shap_values["single_model"][-1].base_values,
        )

        if isinstance(models["single_model"], LinearRegressionModel):
            # coefficients for the last horizon step should be identical
            if univariate:
                multi_models_coefs = models["multi_models"].model.coef_[-1]
            else:
                multi_models_coefs = models["multi_models"].model.coef_[-1 * ts.width :]
            np.testing.assert_almost_equal(
                multi_models_coefs, models["single_model"].model.coef_
            )

            # comparing the expected and computed shap values
            def manual_linear_shap(coefs, inputs):
                """shap_feat_i = (inputs_i - mean(inputs_i)) * coef_i"""
                return np.array(inputs - np.mean(inputs, axis=0)) * coefs

            for i in range(models["single_model"].output_chunk_length):
                if univariate:
                    coefs = models["single_model"].model.coef_
                else:
                    # extracting the appropriate coefficients array
                    coefs = models["single_model"].model.coef_[
                        list(ts.components).index(target_component)
                    ]
                computed_shap = shap_values["single_model"][i].values
                expected_shap = manual_linear_shap(
                    coefs=coefs, inputs=shap_values["single_model"][i].data
                )

                assert expected_shap.shape == computed_shap.shape
                assert (
                    sklearn.metrics.mean_squared_error(expected_shap, computed_shap)
                    < 1e-3
                )
