...
class TestTFTExplainer:
    ...
    def test_prediction_output_collector_validates_cache_shapes_and_presence(self):
        ...
        for name in _TFTPredictionOutputCollector._OUTPUT_ATTRIBUTES:
            setattr(pl_module, name, torch.ones((2, 1), dtype=torch.bfloat16))
        collector.on_predict_batch_end(None, pl_module, outputs, batch, 0)
        assert all(
            output is not None and output.dtype == torch.float32
            for output in collector.collect(expected_num_series=2)
        )
        pl_module._static_covariate_var = None
