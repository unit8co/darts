...
from typing import Optional, Union, Any, List, Sequence, Tuple
...
class ShapExplainer(ForecastingModelExplainer):
    ...
    def __init__(
        ...
        A time series prediction model is a machine learning/statistic model with features beeing the past targets ts, 
        ...
        the indermediate past lags are generated by the same model.
    ...
    def summary_plot(
        ...
        raise_if(
            any([target_name not in self.target_names for target_name in target_names]), 
            "One of the target names doesn't exist in the original background ts.")

        # We suppose for now the output_chunk_length existence
        raise_if(
            max(horizons)>self.model.output_chunk_length-1,
            "One of the horizons is greater than the model output_chunk_length."
        )

        if nb_samples:
            foreground_X_sampled = shap.utils.sample(self.explainers.background_X, nb_samples)
        else:
        ...
        for t in target_names:
            for h in horizons:
                shap_values.append(self.explainers.shap_values_from_X(foreground_X_sampled ,h ,t))
                plt.title('Target: `{}` - Horizon: {}'.format(t, 't+' + str(h)))
                shap.summary_plot(shap_values[-1], foreground_X_sampled,  plot_type=plot_type)
...
class RegressionShapExplainers:
    ...
    def __init__(
        ...
        if (self.target_dim > 1 or self.model.output_chunk_length > 1) and (not self.multioutput):
            self.explainers = {}
    ...
    def shap_values(
        ...
        foreground_past_covariates: Union[TimeSeries, Sequence[TimeSeries]],
        foreground_future_covariates: Union[TimeSeries, Sequence[TimeSeries]],
        horizon: Optional[int]=None, 
        target_name: Optional[str]=None
    ) -> shap._explanation.Explanation:
        
        raise_if_not(
            (len(foreground_series) == len(foreground_past_covariates)) and \
            (len(foreground_past_covariates) == len(foreground_future_covariates)) and \
            (len(foreground_future_covariates) == len(foreground_series)),
            'target, past and future covariates must have the same length.'
        )

        raise_if_not(
            all(foreground_series.time_index == foreground_past_covariates.time_index) and \
            all(foreground_past_covariates.time_index == foreground_future_covariates.time_index) and \
            all(foreground_future_covariates.time_index == foreground_series.time_index),
            'The target, past and future covariates must have the same time index.'
        )

        # We don't recompute if the foreground is the same as the last computation
        if (self.cache_foreground_series != foreground_series) or \
        (self.cache_foreground_past_covariates != foreground_past_covariates) or \
        ...
        return self.shap_values_from_X(
            self.cache_foreground_X,
            horizon, 
            target_name
            )
    ...
    def shap_values_from_X(
        ...
        if (self.target_dim > 1 or self.model.output_chunk_length > 1):
            ...
            # Supported sikit-learn multioutput case
                ...
                shap_values = self.cache_explainers[:,:,horizon*self.target_dict[target_name]+self.target_dict[target_name]]   
        else:
        ...
        for idx, target_ts in enumerate(target_series):
            ...
            if "target" in self.model.lags:
                for lag in self.model.lags["target"]:
                    ...
                    df_X.append(df_tmp.rename(columns={c: c + '_target_lag' + str(lag) for c in df_tmp.columns}))
                ...
                if lags:
                    for lag in lags:
                        ...
                        if idx == 0:
                            ...
                            cov_type = 'fut'
                        df_X.append(df_tmp.rename(columns={c: c + '_' + cov_type + '_cov_lag' + str(lag) for c in df_tmp.columns}))
