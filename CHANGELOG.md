...
- 🔴 Added a new argument `start_format` to `historical_forecasts()`, `backtest()` and `gridsearch` so that `start` can be provided as a 'positional index' (positive or negative) or a point of the time index ('value index', default behavior). [#1866](https://github.com/unit8co/darts/pull/1866) by [Antoine Madrona](https://github.com/madtoinou).
