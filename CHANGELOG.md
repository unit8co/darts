...
- 🔴 Improved the performance of the `TimeSeries.map()` method. For functions that take two arguments, users must now reshape the TimeSeries index explicitly within their function definition. See more information in the `TimeSeries.map()` method documentation. [#2911](https://github.com/unit8co/darts/pull/2911) by [Jakub Chłapek](https://github.com/jakubchlapek)
