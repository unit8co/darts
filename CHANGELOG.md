...
- Added an optional `minimum_length` parameter to local forecasting models (StatsForecast models such as `AutoETS`/`AutoARIMA`, and `ExponentialSmoothing`) to override the conservative default minimum training series length, so these models can be fit on shorter series. [#3167](https://github.com/unit8co/darts/pull/3167) by [Haibin Yu](https://github.com/haiiibin).
