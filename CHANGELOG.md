...
- New Feature: By setting `user_reversible_instance_norm=True`, the reversible [instance normalization](https://github.com/ts-kim/RevIN) is be applied to the target series. This was already available for `PLForecastingModule` and is now also available for `RegressionModel`. By [Jules Authier](https://github.com/julesauthier).
