...
- 🚀 Added support for `start="end"` to `ForecastingModel.historical_forecasts()`, which generates a single forecast per input series starting one step after the end of each series (future-only forecasting). This is also available through `backtest()` and `residuals()`. [#XXXX](https://github.com/unit8co/darts/pull/XXXX) by [Dennis Bader](https://github.com/dennisbader).
