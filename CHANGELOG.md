...
- Added support for `TimeSeries` metadata. You can now embed any type of metadata as a dictionary to your time series: either at series creation with parameter `metadata`, or to an existing series with `TimeSeries.with_metadata()`. We make sure to propagate the metadata in all downstream tasks. In contrast to static covariates, metadata information will not used by Darts' models. [#2656](https://github.com/unit8co/darts/pull/2656) by [Yoav Matzkevich](https://github.com/ymatzkevich), [Jules Authier](https://github.com/authierj) and [Dennis Bader](https://github.com/dennisbader).
...
- 🟠 `TimeSeries.pd_dataframe()` and `TimeSeries.pd_series()` are deprecated and will be removed in Darts version 0.35.0. Use `to_dataframe()` and `to_series()` instead. [#2701](https://github.com/unit8co/darts/pull/2701) by [Jules Authier](https://github.com/authierj)
