...
- Added hyperparameter `skip_resampling` to `TFTModel` that will skip resampling operations on feature embeddings. When `True`, it can greatly increase training and inference efficiency while predictive accuracy remains largely unaffected. [#2898](https://github.com/unit8co/darts/pull/2898) by [Zhihao Dai](https://github.com/daidahao).
