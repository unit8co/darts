...
- Added parameter `val_length` to `ForecastingModel.historical_forecasts()` which allows to extract a validation set of length `val_length` after the end of each training set when `retrain=True`. The validation is then used to fit the underlying forecasting model. [#2894](https://github.com/unit8co/darts/pull/2894) by [Dennis Bader](https://github.com/dennisbader).
